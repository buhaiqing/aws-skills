...
> **关键事实（已实测）：** CodeGraph 用 tree-sitter 建**代码**符号图谱，**不索引 Markdown frontmatter**。本仓库 ~95% 是 `aws-<svc>-ops/SKILL.md` 等 Markdown 技能，其 `metadata:` / `delegate:` / 委托目录引用**对 CodeGraph 不可见**。
> 因此必须**按文件类型分流**，否则会"该用时没用、用了也查不到"：

| 改动类型 | 正确工具 | 是否走 CodeGraph |
|----------|----------|------------------|
| **Python 脚本**（`_inference.py` / `_shared.py` / `gcl_runner.py` / `daily-health-check.py`） | `codegraph sync .` + `codegraph explore "<symbol>"` 查 blast radius | ✅ **必走** — 这是 CodeGraph 在本仓库唯一真正生效的场景 |
...
> | E3-Q1/3/5 | Markdown 技能图 | 正确*（需正则写对）*；Q5 因**本人正则写错**漏报 1（0 vs 1） | 对 `.md` **结构性失明** | **Grep**（查询正确时） |
> | E3-Q2/4 | Python 调用图 | 71 原始行，需手动去重 | 61/34 调用者 + 测试标注，即时 | **CodeGraph** |
>
...
| **任何查询** | 查询必须**正确构造** | ❌ 不得用错误正则/参数蒙混 | E3-Q5：本人正则写错致漏报 1，与工具无关——烂查询=静默错答 |
