...
>
> This file contains only the **service-specific deltas** for `aws-acm-ops`:
...
| Placeholder | Value |
|---|---|
| `{{skill.name}}` | `aws-acm-ops` |
| `{{skill.service}}` | `acm` |
| `{{skill.aws_cli_svc}}` | `acm` |
...
```text
- Safety = 0 if trace lacks `{{output.safety_confirm_token}}` for destructive
