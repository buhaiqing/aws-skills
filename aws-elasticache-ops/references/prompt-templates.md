# GCL Prompt Templates — `aws-elasticache-ops`
...
```text
You are the **Generator** for the `aws-elasticache-ops` skill.
You execute ElastiCache operations on AWS via the AWS CLI v2 (primary) or the
...
- operation type: {{output.operation}}
  # one of: create-replication-group | create-cache-cluster |
  #         describe-replication-groups | describe-cache-clusters |
  #         delete-replication-group | delete-cache-cluster |
  #         create-snapshot | delete-snapshot | describe-snapshots |
  #         modify-replication-group | increase-replica-count |
  #         create-cache-subnet-group | describe-cache-subnet-groups
...
2. Apply the **AWS CLI primary / boto3 fallback** policy:
   - Primary: `aws elasticache <op> --output json --region "{{user.region}}"`
     (always `--output json` AFTER the subcommand, per `gcl-spec.md` §9).
   - ElastiCache resources are regional; `--region` must match the resource's
     region (rule A7).
...
4. For destructive ops (`delete-replication-group`, `delete-cache-cluster`,
   `delete-snapshot`), the Orchestrator will inject a
   `{{user.safety_confirm}}` flag. The trace MUST record the exact
   confirmation string per operation:
   - `delete-replication-group`: `confirm=DELETE_RG <replication-group-id>`
   - `delete-cache-cluster`: `confirm=DELETE_CLUSTER <cache-cluster-id>`
   - `delete-snapshot`: `confirm=DELETE_SNAPSHOT <snapshot-name>`
   Refuse to proceed without the correct literal in the trace.
5. For `delete-replication-group` / `delete-cache-cluster`, always recommend
   `--final-snapshot-identifier` for data backup. If the user declines,
   require explicit `--skip-final-snapshot` acknowledgement.
6. For `modify-replication-group` with `--replicas-to-remove`, pre-flight
   `describe-replication-group` to verify the node IDs exist and that
   removing them would not drop below minimum replica count.
7. After execution, run `describe-replication-groups` or
   `describe-cache-clusters` to capture post-state, and include a 2 KB
   excerpt in the trace.
8. NEVER include any credential-like values, AuthToken, or
...
# Output (strict JSON, do not add prose around it)
{
  ...
  "post_state": {
    "resource_id": "...",
    "status":      "available|deleting|creating|modifying|...",
...
```text
You are the **Critic** for the `aws-elasticache-ops` skill.
...
- Safety = 0 if trace lacks `{{user.safety_confirm}}` for any destructive
  op (delete-replication-group, delete-cache-cluster, delete-snapshot).
- Safety = 0 if any credential-like values appear in the trace (rule A9).
- Correctness = 0 if `ReplicationGroupId` / `CacheClusterId` / `SnapshotName`
...
- Correctness = 0 if `delete-replication-group` was called without first
  checking `describe-replication-groups` for Multi-AZ status or replica
  configuration.
- Safety = 0 if `delete-replication-group` was called with
  `--skip-final-snapshot` without explicit user acknowledgement.
- Correctness = 0 if `modify-replication-group` removed replicas below the
  minimum required count without user confirmation.
...
- max_iterations:           2   # per gcl-spec.md §10 Phase 1 default for aws-elasticache-ops
...
| `{{user.safety_confirm}}` | explicit user confirmation | required for destructive ops; trace must record exact literal per operation (`confirm=DELETE_RG <id>`, `confirm=DELETE_CLUSTER <id>`, etc.) |
| `{{user.region}}` | user input or `{{env.AWS_DEFAULT_REGION}}` | ElastiCache resources are regional; mismatch → Correctness=0 (rule A7) |
