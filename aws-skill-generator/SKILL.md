...
| **C7** | **分层契约 (Layering Contract)** | 若 `metadata.type == composite`（或既有等价值 `orchestrator-meta`），则必含 `delegate` 且其指向的 `aws-<svc>-ops` 目录真实存在；`base` 技能可选填 `provides` | composite / orchestrator-meta 缺失 delegate 或 delegate 目录不存在则 HALT 报告 |
...
| `{{skill.type}}` | `base` \| `composite`(或既有等价值 `orchestrator-meta`) (from `metadata.type` in SKILL.md frontmatter) |
