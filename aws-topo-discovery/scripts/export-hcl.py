#!/usr/bin/env python3
"""CLI entry point for aws-topo-discovery HCL export.
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Export AWS cloud resources as Terraform HCL",
    )
    ...
    parser.add_argument("--provider-version", default="5.50.0",
                        help="AWS Provider version (default: 5.50.0)")
...
def main():
    ...
    if args.assume_role:
        ...
        result = subprocess.run(
            ["bash", "-c", f"source aws-topo-discovery/scripts/sts-helper.sh "
                          f"--role-arn '{args.assume_role}' && "
                          f"echo 'AK=$AWS_ACCESS_KEY_ID'"],
...
def _gen_import_script(ordered, block_lookup, region):
    ...
    for item in ordered:
        ...
        rid = data.get('VpcId', data.get('InstanceId',
              data.get('SubnetId', data.get('GroupId',
              data.get('DBInstanceIdentifier', '?')))))
        lines.append(f"# terraform import '{spec.terraform_type}.{block_name}' {rid}")
    return "\n".join(lines) + "\n"
