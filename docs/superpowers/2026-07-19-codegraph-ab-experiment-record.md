...
| **方法 B (CodeGraph)** | **42 个去重调用者**跨 11 文件 + `⚠️ no covering tests found` 标注 + 按文件聚合 | **0.134s** | ✅ **信息更丰富（enrichment）** |

**结论 E2**（已 re-verified，rev #2）：Grep 返回 72 个原始非测试调用点；CodeGraph 返回 42 个去重调用者 + `⚠️ no covering tests found` 标注（enrichment：去重 + 测试覆盖标注）。
两方协议不同（Grep 报原始行、CodeGraph 报去重调用者），"CodeGraph 严格更优 / Grep 不完整"属于过度推断 —— 增益是 **enrichment（去重 + 测试标注），不是 recall 正确性**。两组 count 因协议不同不可直接相减比较。
